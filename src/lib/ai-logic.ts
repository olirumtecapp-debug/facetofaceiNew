...
export const getAIPalpite = (
  ...
  if (difficulty === "Fácil") {
    if (remainingCharacters.length === 1 && Math.random() > 0.6) return remainingCharacters[0] || null;
    return null;
  }
  ...
  if (difficulty === "Médio") {
    ...
    if (remainingCharacters.length === 2 && Math.random() > 0.7) return remainingCharacters[Math.floor(Math.random() * 2)] || null;
    return null;
  }
  ...
  if (difficulty === "Difícil") {
    if (remainingCharacters.length === 1) return remainingCharacters[0] || null;
    if (remainingCharacters.length <= 3 && Math.random() > 0.5) {
