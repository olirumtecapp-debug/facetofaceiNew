...
function Index() {
  ...
  const [playerName, setPlayerName] = useState(() => {
    return (typeof window !== 'undefined' ? localStorage.getItem("ftf_player_name") : "") || "";
  });
  const [isConnecting, setIsConnecting] = useState(false);
  const [roomData, setRoomData] = useState<any>(null);
  const [players, setPlayers] = useState<any[]>([]);
  
  const createRoomFn = createRoom;
  const joinRoomFn = joinRoom;
        ...
        <div className="grid w-full max-w-md gap-4">
          {(["Fácil", "Médio", "Difícil"] as Difficulty[]).map((d) => (
            <button
              ...
              className="group relative overflow-hidden rounded-xl border-2 border-blue-400/50 bg-gray-800/80 p-5 text-xl font-black italic tracking-wider transition-all hover:scale-105 hover:bg-gray-700 hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] active:scale-95 cursor-pointer"
            >
        ...
        <div className="flex w-full max-w-2xl flex-col gap-6 md:flex-row">
          {(["AZUL", "VERMELHO"] as const).map((c) => (
            <button
              ...
              className="group flex-1 rounded-2xl border-4 p-8 transition-all hover:scale-105 active:scale-95 cursor-pointer"
              style={{
          ...
          <div className="w-full max-w-md space-y-4">
            ...
            <div className="rounded-xl border border-white/10 bg-[#11151d] p-5">
              ...
              <button
                ...
                className="w-full rounded-lg bg-[#1e62ec] py-3 font-black uppercase tracking-widest border-2 border-blue-400/50 transition-all hover:brightness-125 active:scale-95 disabled:opacity-50 cursor-pointer"
              >
            ...
            <div className="rounded-xl border border-white/10 bg-[#11151d] p-5">
              ...
              <button 
                ...
                className="w-full rounded-lg bg-[#e52e2e] py-3 font-black uppercase tracking-widest border-2 border-[#ff4444]/50 transition-all hover:brightness-125 active:scale-95 disabled:opacity-50 cursor-pointer"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-3 backdrop-blur-sm">
          <div className="flex max-h-[95dvh] w-full max-w-4xl flex-col rounded-2xl border-2 border-yellow-400/30 bg-[#0b0e14] p-4 shadow-[0_0_40px_rgba(0,0,0,0.5)] sm:p-6">
              ...
              <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 sm:gap-4 lg:gap-6">
                {CHARACTERS.map((c) => (
                  <button
                    ...
                    className="group relative flex flex-col items-center transition-all hover:z-10 cursor-pointer"
                  >
            ...
            <button
              ...
              className="mt-4 w-full rounded-xl border-2 border-gray-500/50 bg-gray-800 py-3 font-black tracking-widest text-white transition-all hover:bg-gray-700 hover:scale-[1.02] active:scale-95 cursor-pointer"
            >
              FECHAR
            </button>
          </div>
              ...
              return (
                <div className="flex flex-col items-center p-4 text-gray-900">
                  ...
                  <div className="grid grid-cols-1 gap-y-1.5 w-full text-left font-bold text-[10px] sm:text-xs bg-white/40 p-3 rounded-xl border border-black/5">
                    <p className="flex justify-between border-b border-black/5 pb-1"><span className="text-gray-500 uppercase text-[8px]">Profissão:</span> {details?.profession || "Artista"}</p>
                    <p className="flex justify-between border-b border-black/5 pb-1"><span className="text-gray-500 uppercase text-[8px]">Personalidade:</span> {details?.personality || "Criativo"}</p>
                    <p className="flex justify-between border-b border-black/5 pb-1"><span className="text-gray-500 uppercase text-[8px]">Hobbies:</span> {details?.hobbies?.join(", ") || "Jogos"}</p>
                    <p className="flex flex-col gap-0.5"><span className="text-gray-500 uppercase text-[8px]">Sobre:</span> <span className="text-[11px] leading-tight text-gray-800 italic">{details?.bio || "Personagem clássico FTF."}</span></p>
                  </div>
                  <button
                    onClick={() => setSelectedCharId(null)}
                    className="mt-6 w-full rounded-xl border-2 border-gray-500/50 bg-gray-800 py-4 font-black tracking-[0.2em] text-white transition-all hover:bg-gray-700 hover:scale-[1.05] active:scale-95 cursor-pointer"
                  >
...
function BackButton({ onClick, className }: { onClick: () => void; className?: string }) {
  return (
    <button
      onClick={onClick}
      className={`group flex items-center gap-2 px-6 py-2 rounded-xl border-2 border-gray-400/30 bg-gray-800/50 font-black uppercase tracking-widest text-gray-400 transition-all hover:text-yellow-400 hover:border-yellow-400/50 hover:scale-105 active:scale-95 cursor-pointer ${className}`}
    >
